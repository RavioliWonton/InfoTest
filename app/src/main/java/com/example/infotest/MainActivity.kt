...
class MainActivity : ComponentActivity() {
    ...
    private val qPermissionArray = permissionArray.asList().plusElement(Manifest.permission.ACCESS_MEDIA_LOCATION)
        .minusElement(Manifest.permission.WRITE_EXTERNAL_STORAGE).toTypedArray()
    private val sPermissionArray = qPermissionArray.asList().plus(arrayOf(Manifest.permission.READ_MEDIA_AUDIO,
    ...
    private val permissionStatus by lazy {
        (if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) sPermissionArray
        else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) qPermissionArray
        else permissionArray).map { ContextCompat.checkSelfPermission(this, it) ==
                PackageManager.PERMISSION_GRANTED }.all { it }
    }
    private val permissionLauncher = registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { map ->
        if (map.values.all { it }) startFetch()
    ...
    override fun onStart() {
        super.onStart()
        if (permissionStatus) startFetch()
    ...
    private fun startFetch() {
                        ...
                        if (availability.isLocationAvailable) locationServices.lastLocation.addOnSuccessListener { lastKnown ->
                            ...
                            if (lastKnown != null && (GlobalApplication.gps?.time ?: -1L) < LocationCompat.getElapsedRealtimeNanos(lastKnown)) {
                                GlobalApplication.gps = GPS(
                                    lastKnown.latitude.toString(), lastKnown.longitude.toString(),
        ...
        lifecycleScope.launch(extensionCreationContext) {
            delay(5.seconds)
            text = "正在抓取……"
            createExtensionModel().toJson().saveFileToDownload("model-${LocalDateTime.now()}.txt", contentResolver)
